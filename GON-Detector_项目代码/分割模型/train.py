...
def dice_loss2(y_pred,y_true):
    ...
    #训练要除以8
    return 1.-(2. * intersection +smooth ) / (fluid.layers.reduce_sum(y_true_f) + fluid.layers.reduce_sum(y_pred_f) + smooth)
...
def train(model):
    ...
    with fluid.dygraph.guard():
        ...
        for epoch in range(epoch_num):
            ...
            print('epoch is {}, train loss is {}, val loss is {}'.format(epoch+1,train_loss.numpy(),val_loss.numpy()))
            if (epoch+1) % 10 == 0:
...
if __name__ == '__main__':
    ...
    with fluid.dygraph.guard():
        ...
        print(a.shape)
        a=fluid.layers.flatten(a,2)
        print(a.shape)
        print(a.numpy())
        a=fluid.layers.reshape(a,[a.shape[0]])
        print(a.shape)
        print(a.numpy())
        '''
        #测试模型效果
